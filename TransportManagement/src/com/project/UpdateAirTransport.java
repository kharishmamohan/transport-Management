...
/**
 * Servlet implementation class UpdateAirTransport
 */
@WebServlet("/UpdateAirTransport")
public class UpdateAirTransport extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(button.equals("Update")) {
			try {
                ...
                PreparedStatement ps = c1.prepareStatement("update airtransport set to_location = ?, vehicle_id = ?, weightofcargo = ? , journey_time = ? ,price = ?, dateofjourney = ? where from_location = ?");
				...
				if(k>0) {
					pw.println("updated airtransport details");
